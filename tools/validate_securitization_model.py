"""Validate the institutional securitization workbook contract."""
...
REQUIRED_LABELS = {
    "Monthly Collateral Cash Flows",
    "Monthly Priority of Payments",
    "Senior interest shortfall",
    "Mezz interest shortfall",
    "OC ratio",
    "Senior Ending Balance Sensitivity",
    "WAL",
}
...
def validate(path: Path) -> list[str]:
    ...
    if len(formulas) < 600:
        errors.append(f"formula depth below contract: {len(formulas)} < 600")
    ...
    if "Checks" in workbook.sheetnames:
        ...
        if len(checks) < 8:
            errors.append("insufficient model checks")
    return errors

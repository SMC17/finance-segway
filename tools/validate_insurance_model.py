"""Validate the institutional insurance and actuarial workbook contract."""
...
def validate(path: Path) -> list[str]:
    ...
    if len(formulas) < 240:
        errors.append(f"formula depth below contract: {len(formulas)} < 240")
    if "Chain Ladder" in workbook.sheetnames:
        chain = workbook["Chain Ladder"]
        if chain["L6"].value != 1 and chain["L6"].value != "=1":
            errors.append("tail cumulative development factor must equal one")

...
def apply_manifest(manifest_path: Path, root: Path) -> dict[str, Any]:
    ...
    if "RefreshLog" in workbook.sheetnames:
        ...
        row = 5
        while any(refresh.cell(row, column).value not in (None, "") for column in range(2, refresh.max_column + 1)):
            row += 1
        entry = manifest.get("refresh") or {}
        values = {
            "date": entry.get("date", manifest["as_of"]),
            "trigger": entry.get("trigger", "Manifest generation"),
            "source snapshot": entry.get("source_snapshot", f"repo://standards/benchmark_cases/{manifest['id']}.json"),
            "what changed": entry.get("what_changed", f"Applied manifest {manifest['id']}"),
            "reviewer / challenge": entry.get("reviewer_notes", "Generated reproducibly"),
            "reviewer notes": entry.get("reviewer_notes", "Generated reproducibly"),
            "next check": entry.get("next_check", "On builder or contract change"),
        }
        ...
        for candidate in range(5, row):
            if refresh.cell(candidate, headers.get("trigger", 3)).value == values["trigger"]:
                for column in range(2, refresh.max_column + 1):

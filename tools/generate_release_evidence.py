...
def build_evidence(root: Path, release_id: str,
...
def main() -> int:
    ...
    parser.add_argument(
        "--model-ids",
        default="03,04,09,14,18,19,20,21,22",
        help="comma-separated inventory model IDs",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("standards/releases/flagship-2.1.0.json"),
    )
    args = parser.parse_args()
    root = args.root.resolve()
    model_ids = [item.strip() for item in args.model_ids.split(",") if item.strip()]
    evidence = build_evidence(root, args.release_id, model_ids)

"""Validate the institutional quantitative/systematic workbook contract."""
...
REQUIRED_LABELS = {
    ...
    "Gross, Net, and Risk-Adjusted Performance",
    "Walk-Forward Validation Folds",
    "Cost and Capacity Sensitivity",
    "Historical expected shortfall",
    "Publication lag control enabled",
}
...
def validate(path: Path) -> list[str]:
    ...
    if len(formulas) < 500:
        errors.append(f"formula depth below contract: {len(formulas)} < 500")
    ...
    if "Checks" in workbook.sheetnames:
        ...
        if len(checks) < 9:
            errors.append("insufficient research checks")
    return errors

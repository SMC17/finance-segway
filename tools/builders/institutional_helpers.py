...
def add_cover(wb: Workbook, title_text: str, fields: Sequence[tuple[str, object]], *, scenario: bool = True):
    ...
    ws["B13"].font = Font(name="Arial", size=10, bold=True, color=BLUE)
    ws["B13"].fill = PatternFill("solid", fgColor=YELLOW)
    ws["B15"].font = Font(name="Arial", size=10, bold=True, color=GREEN)
    set_widths(ws, {"A": 4, "B": 34, "C": 42, "D": 12, "E": 12, "F": 12})
    ws.freeze_panes = "A3"
...
def add_refresh_log(wb: Workbook):
    ...
    set_widths(ws, {"A": 4, "B": 14, "C": 20, "D": 36, "E": 36, "F": 16})
    ws.freeze_panes = "A5"
    ws.sheet_view.showGridLines = False
    return ws

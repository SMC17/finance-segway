...
That mismatch used to show up as **NAV roll-forward reconciles = FAIL** (Overall: BREACH) with a computed-vs-reported residual of roughly $77.8mm (~1.3% of ending NAV). It was not the fee model's simplification: the fund's own Statement of Changes in Net Assets opens with **Net investment loss $(113,289,251)** (Statement of Operations: Total Expenses $175,929,631), and the case sourced five of that statement's six components while leaving this one to the template's modelled management fee of $35.5mm. The $77.8mm residual was exactly the difference between the two. The disclosed figure is now sourced into the roll-forward's `Disclosed net investment loss` line, and the case reconciles to the reported ending NAV to the dollar: **all six Checks PASS, Overall PASS**.
...
This fund uses open-end/interval subscription-redemption mechanics (contributions + reinvested distributions + redemptions) rather than private-equity-style capital calls. That is a real difference, but it is not a missing template slot: the roll-forward's capital line takes the period's **net** capital share transactions, which is how the HLPAF case already sources it. The FY2023 redemptions of $213.5mm and the fund's disclosed net investment loss of $40.6mm are both captured in this case's snapshot and are now both wired in, so the roll-forward reconciles. What remains is real and undisguised: **single-fund concentration = BREACH** (Point72 at ~27.4% of the look-through portfolio, over the 20% policy limit), **FoF-level called capital vs. commitments = FAIL**, **Overall: BREACH** — the disclosed concentration and capital position, no longer mixed in with a plumbing gap.
...
- Neither real public case trips the NAV roll-forward reconciliation check any more. Both previously did, and in both cases the cause was the same: components disclosed in the fund's own Statement of Changes in Net Assets were not carried into the roll-forward — the net investment loss in both, and the period's redemptions in SkyBridge's. `fof-public-hlpaf-2026` now passes every check; `fof-public-skybridge-fy2023-stress` still shows Overall BREACH, driven only by its real, disclosed single-fund concentration and called-capital position. That is what makes the pair genuinely differentiated rather than two cases failing the same plumbing.

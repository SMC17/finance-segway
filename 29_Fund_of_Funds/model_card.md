...
- Risk tier: Tier 1
- Declared maturity: **M1** (correct skeleton, one verified core identity — see "Why M1, not M2" below)
...
- Public cases: none yet. See "Why M1, not M2" below.

## Why M1, not M2

`tools/validate_model_inventory.py` now requires (as of this session's #36 fixes) at least one real, LibreOffice-recalculated public case in `standards/public_cases/index.json` before a model can declare M2+. Sourcing one for this domain requires primary or regulator-filed data from a real, publicly-reporting fund-of-funds (e.g. HarbourVest Global Private Equity, Pantheon International) — attempted via HVPE's own factsheet PDF and the UK FCA's National Storage Mechanism filing archive; both returned HTTP 403 in this session's network environment, the same bot-protection pattern that blocked SEC EDGAR access in an earlier pass of this repo's work. Rather than substitute a WebSearch-synthesized figure that couldn't be independently verified by fetching the underlying document, this domain ships at M1 with the mechanics fully built, checked, and internally consistent, and the real-instance gap stated plainly here instead of papered over.

## Inputs and sources

- Source register: `29_Fund_of_Funds/sources/source_register.csv` (present, not yet populated with a real case)
- Frozen snapshots: `29_Fund_of_Funds/sources/snapshots/` (empty pending a sourced instance)
...
- No real public instance yet (see "Why M1, not M2").

...
| Composition weight sum (7 disclosed sectors + disclosed balancing line) | 1.000 | 7 sector weights (102.6% of net assets, investments only) plus the filing's own `OTHER ASSETS LESS LIABILITIES` of (2.6)% | inside [0.90, 1.01] -> **PASS** (see Findings) |

### Findings (kweb case)

| ID | Severity | Finding | Evidence | Disposition |
|---|---|---|---|---|
| ETF-03 | Informational | KWEB's SEC N-CSR sector weightings sum to 102.6%, not ~100%, because they are percent-of-net-assets applied to **investments only**. The fund's Consolidated Schedule of Investments closes the identity three lines below the sector table: `TOTAL INVESTMENTS - 102.6% ... OTHER ASSETS LESS LIABILITIES - (2.6)% ... NET ASSETS - 100%`. The fund is carrying 2.6% of net liabilities; nothing is double-counted. The same filing shows this identity ranging from 67.6% to 102.6% across the trust's other funds, so a 90%-101% band mis-specifies normal fund reporting rather than detecting an anomaly. | `tools/data_fabric/out/KWEB_sec_ncsr_annual_report.json`, quoted_text field (both lines captured) | Both disclosed halves are carried in the composition grid, so it reconciles to exactly 100.0% and the sector-weight check passes on its merits. This adds a second disclosed figure from the same filing -- it does not smooth, round, or adjust any number. An earlier revision of this case carried only the first half, attributed the 102.6% to securities-lending collateral double-counting (a guess, and the wrong one), and recorded the resulting REVIEW as expected behaviour. |

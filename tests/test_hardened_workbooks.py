...
class HardenedWorkbookTests(unittest.TestCase):
    ...
    def test_all_four_workbooks_build(self):
        self.assertEqual(
            {path.name for path in self.directory.glob("*.xlsx")},
            {contract["filename"] for contract in validate_hardened_workbooks.CONTRACTS.values()},
        )

    def test_all_contracts_pass(self):
        self.assertEqual(self.report["status"], "PASS", msg=self.report["errors"])
        self.assertEqual(self.report["models"], 4)
    ...
    def test_no_inventory_promotion_is_implicit(self):
        self.assertIn("required before inventory maturity", self.report["promotion_statement"])

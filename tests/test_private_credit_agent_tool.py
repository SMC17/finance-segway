...
class PrivateCreditAgentToolTests(unittest.TestCase):
    ...
    def test_ares_fixture_only_submits_facts_with_real_provenance(self):
        # Every fact the fixture submits must carry a real EDGAR source URL
        # and a transformation note explaining the portfolio-to-single-
        # facility mapping -- this fixture must never invent an unsourced
        # revenue/EBITDA number just to make Checks show PASS.
        inp = ares_fixture()
        self.assertEqual(inp.instance_slug, "public_ares_capital_2024")
        ...
        for label, prov in inp.provenance.items():
            ...
            self.assertIn("companyfacts", prov.source_url)
